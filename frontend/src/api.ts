...
export const listReports = (projectId: number) => API.get(`/projects/${projectId}/reports`)
export const downloadReport = (projectId: number, reportId: number) =>
  API.get(`/projects/${projectId}/reports/${reportId}`, { responseType: 'blob' })
export const deleteReport = (projectId: number, reportId: number) => API.delete(`/projects/${projectId}/reports/${reportId}`)
...
export const getQC = (projectId: number, datasetId: number, selectedGroups: string[] = [], groupOrder?: string[], tickSize?: number, axisLabelSize?: number, groupColors?: Record<string, string>) => {
  const params = new URLSearchParams()
  selectedGroups.forEach((g) => params.append('selected_groups', g))
  groupOrder?.forEach((g) => params.append('group_order', g))
  if (tickSize !== undefined) params.append('tick_size', String(tickSize))
  if (axisLabelSize !== undefined) params.append('axis_label_size', String(axisLabelSize))
  if (groupColors && Object.keys(groupColors).length > 0) params.append('group_colors', JSON.stringify(groupColors))
  return API.get(`/analysis/${projectId}/dataset/${datasetId}/qc`, { params })
}
export const exportQCExcel = (projectId: number, datasetId: number, selectedGroups: string[] = [], groupOrder?: string[], groupColors?: Record<string, string>) => {
  const params = new URLSearchParams()
  selectedGroups.forEach((g) => params.append('selected_groups', g))
  groupOrder?.forEach((g) => params.append('group_order', g))
  if (groupColors && Object.keys(groupColors).length > 0) params.append('group_colors', JSON.stringify(groupColors))
...
export const exportQCPdf = (projectId: number, datasetId: number, data: { selected_groups?: string[]; group_order?: string[]; selected_plots?: string[]; primary_comparison?: string; prepared_for?: string; prepared_by?: string; report_contents?: string; report_type?: string; subtitle?: string; description?: string; cover_style?: string; font_family?: string; tick_size?: number; axis_label_size?: number; group_colors?: Record<string, string>; plots_per_page?: number; plot_layout?: Record<string, string>; plot_images?: Record<string, string>; save_to_s3?: boolean }) =>

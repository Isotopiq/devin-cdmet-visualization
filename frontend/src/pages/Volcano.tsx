...
import { listProjects, listDatasets, runStats, generatePlot } from '../api'
...
export default function Volcano() {
  ...
  const generate = async () => {
    ...
    const statsRes = await runStats(Number(projectId), Number(datasetId), { test: 't_test', group_a: 'control', group_b: 'treatment', paired: false, multiple_testing: 'fdr_bh', alpha: 0.05 })
    const res = await generatePlot(Number(projectId), Number(datasetId), { plot_type: 'volcano', parameters: { stats: statsRes.data.results } })
    setFigure(res.data)
  }

  return (
    <div>
      <h1 className="text-2xl font-bold mb-4 text-gray-900 dark:text-white">Volcano Plot</h1>
